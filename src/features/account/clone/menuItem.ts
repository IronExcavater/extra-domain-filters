...
// Copied from public/bin.svg (SVG Repo), split back into its individual subpaths so each can
// carry fill="currentColor" like Domain's own icons do, instead of the hardcoded #000000 the
// source file uses. Replaces the ribbon/star icon the Shortlist item is cloned with.
const BIN_ICON_PATHS = [
...
export async function cloneMenuItem(
    ...
    icon.replaceChildren(...BIN_ICON_PATHS.map(d => {
